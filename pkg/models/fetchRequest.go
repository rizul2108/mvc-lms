...
func FetchRequests(db *sql.DB, username string) ([]types.Request, string) {
	...
	rows, err := db.Query("SELECT request_id, book_id, state, req_type, req_date FROM requests WHERE user_id=?", userID)
	if err != nil {
		fmt.Println(err)
...
func FetchAllRequests() ([]types.Request, string) {
	...
	var userID int
	rows, err := db.Query(`
		SELECT r.request_id, r.book_id, r.state, r.req_type, b.title, u.full_name, r.user_id, r.req_date
		FROM requests r
